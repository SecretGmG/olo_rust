...
! 
  if (yy.eq.RONE.and.odd.eq.0) then
    if (ntwo.ne.0.and.error_dilog_r.ne.0) then
      errorcode = errorcode+1
...
!
  if (j1.ne.j2) then
    if (r1.eq.r2.and.error_dilog2_r(1).eq.1) then
      errorcode = errorcode+1
      if (eunit.ge.0) write(eunit,*) 'ERROR in OneLOop dilog2_r 1: ' &
        ,'j1,j2,r1-r2',j1,j2,',',trim(myprint(r1-r2)),', returning 0'
      rslt = 0
...
!
  if (r1.lt.eps) then
    if (r2.lt.eps.and.error_dilog2_r(2).eq.1) then
      errorcode = errorcode+1
      if (eunit.ge.0) write(eunit,*) 'ERROR in OneLOop dilog2_r 2: ' &
        ,'r1,r2 =',trim(myprint(r1)),',',trim(myprint(r2)),', returning 0'
      rslt = 0
...
!      write(*,*) 'dilog2_r ||1-y1|/|1-y2|-1|>0.1' !DEBUG
    ...
    elseif (oo.eq.0.and.ro1.lt.eps) then
      if (nn.ne.0.and.error_dilog2_r(3).eq.1) then
        errorcode = errorcode+1
        if (eunit.ge.0) write(eunit,*) 'ERROR in OneLOop dilog2_r 3: ' &
          ,'r1,oo,nn =',trim(myprint(r1)),',',oo,nn,', putting nn=0'
      endif
      if (ro2.lt.eps) then
        rslt = -1
!        write(*,*) 'dilog2_r |1-y1|' !DEBUG
    ...
    elseif (oo.eq.0.and.ro2.lt.eps) then
      if (nn.ne.0.and.error_dilog2_r(4).eq.1) then
        errorcode = errorcode+1
        if (eunit.ge.0) write(eunit,*) 'ERROR in OneLOop dilog2_r 4: ' &
          ,'r2,oo,nn =',trim(myprint(r2)),',',oo,nn,', putting nn=0'
      endif
      y2=1-eps ;nn=0 ;logr2=0 ;r2=1-eps
    endif
...
!      write(*,*) 'dilog2_r |logr1/logr2-1|>0.1',logr1,logr2 !DEBUG
    ...
    elseif (rr.lt.eps) then
      ...
      ii = nn*ii
      if (ii.ne.0.and.error_dilog2_r(5).eq.1) then
        errorcode = errorcode+1
        if (eunit.ge.0) write(eunit,*) 'ERROR in OneLOop dilog2_r 5: ' &
          ,'r1,r2,nn =',trim(myprint(r1)),',',trim(myprint(r2)),',',nn &
          ,', putting nn=0'
      endif
      rslt = -olog1(y2,2*oo)
