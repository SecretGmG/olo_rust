...
!      call olo_unit( 6 ,'printall' )
      call olo_switch('error_dilog2_r_2-off')
!
      next = 'next'
      do while (trim(next).eq.'next')
        read(5,*,end=999) nn ,mu
        call olo_scale( mu )
        ...
        read(5,'(A)') next
        do while(trim(next).ne.'next')
          write(*,*) next
          read(5,'(A)',end=999) next
          if (trim(next).eq.'stop') goto 999
        enddo
      enddo
  999 continue
      end
